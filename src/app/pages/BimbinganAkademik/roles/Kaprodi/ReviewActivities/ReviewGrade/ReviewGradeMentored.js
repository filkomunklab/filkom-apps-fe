...
const data = Array.from(Array(15).keys()).map((item, index) => ({
  submission_date: `Feb 18, 2022`,
  nim: `105022010000`,
  name: `Katong, Jesica Imanuella`,
  supervisor_name: `Kalalo, Sherina Brenada`,
...
const ReviewGradeMentored = () => {
  ...
  const TableItem = ({ item, index }) => {
    ...
    const handleButtonNavigate = () => {
      navigate(
        `/bimbingan-akademik/kaprodi/review-activities/grade/${item.nim}`
      );
    };
    return (
      <TableRow
...
export default ReviewGradeMentored;
