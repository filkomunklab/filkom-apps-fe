...
const data = Array.from(Array(15).keys()).map((item, index) => ({
  ...
  year: `2021`,
  status: `Active`,
  status_krs: `Not Yet Filled`,
}));

const ListStudentNotPreRegistred = () => {
  ...
  const TableItem = ({ item, index }) => {
    ...
    const handleButtonNavigate = () => {
      navigate(
        `/bimbingan-akademik/dekan/history/activity/list-courses/list-student/${item.code}/student-preregistration`
      );
    };
    return (
      <TableRow
    ...
    <Div>
      <Div role="presentation" onClick={handleClick}>
        <Breadcrumbs aria-label="breadcrumb">
          ...
          <Typography color="text.primary">List Students</Typography>
        </Breadcrumbs>
      </Div>
...
export default ListStudentNotPreRegistred;
