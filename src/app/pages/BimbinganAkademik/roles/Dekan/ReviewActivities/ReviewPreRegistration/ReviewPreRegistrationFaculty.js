...
const ReviewPreRegistrationFaculty = () => {
  const [filter, setFilter] = useState([]);
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(10);

  const getPreRegisList = async() =>{
    try{
      const headers = {
  ...
  const TableHeading = ({ index }) => {
    ...
    return (
      <TableRow sx={{ backgroundColor: "#1A38601A" }}>
        ...
        <TableCell sx={[style]}>Credit Taken</TableCell>
    ...
    <Div sx={{ paddingTop: 2 }}>
      <Grid container spacing={2}>
        <Grid display={"flex"} alignItems={"flex-end"} item md={6}>
          ...
          >
            Review Student Faculty Pre-Registration
          </Typography>
        </Grid>
...
export default ReviewPreRegistrationFaculty;
