...
const DaftarPengujianSkripsiAnggotaPenelis = () => {
  ...
  const handleSearch = () => {
    const results = daftarPengujianSkripsi.semesterData.flatMap(
      (semesterData) =>
    ...
    const fetchDaftarPengujianSkripsiData = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL_API}/group/skripsi-list-member`,
          {
            headers: {
  ...
  return (
    <Div>
      ...
      >
        ...
        {/* Header End */}
        {/* Semester Start */}
        {daftarPengujianSkripsi?.semesterData?.length > 0 ? (
          <Div
            sx={{
              display: "inline-flex",
              flexDirection: "column",
...
export default DaftarPengujianSkripsiAnggotaPenelis;
