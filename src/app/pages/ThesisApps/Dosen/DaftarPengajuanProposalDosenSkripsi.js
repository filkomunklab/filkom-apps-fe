    ...
    const fetchDaftarPengajuanProposalData = async () => {
      try {
        const response = await axios.get(
          `${BASE_URL_API}/group/proposal-list-mk`,
          {
            headers: {
      ...
      >
                    ...
                    {searchResults.map((proposal, proposalIndex) => (
                      <TableRow key={proposalIndex}>
                        ...
                        <TableCell>
                          <Typography
                            component={Link}
                            to={`/sistem-informasi-skripsi/daftar-pengajuan-proposal-dosen-skripsi/beranda/${proposal.group_id}/DOSEN_MK`}
                            sx={{
                              textDecoration: "none",
                              color: "blue",
                            }}
        ...
        {daftarPengajuanProposal?.semesterData?.length > 0 ? (
          ...
          >
            ...
            >
              Belum ada mahasiswa yang mengajukan Proposal.
            </Typography>
          </Div>
        )}
