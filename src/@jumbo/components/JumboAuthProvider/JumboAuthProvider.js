...
import useRouteAccessMatch from "@jumbo/hooks/useRouteAccessMatch";
...
const init = () => {
  ...
  return {
    ...
    redirectNoAccess: config.authSetting.redirectNoAccess,
  };
};
...
const JumboAuthProvider = ({ children, ...restProps }) => {
  ...
  const isUserHadAccess = useRouteAccessMatch(routes);
  ...
  React.useEffect(() => {
    if (!authOptions.authToken) {
      ...
      isUserHadAccess.then((res) => {
        if (!res) {
          navigate(authOptions?.redirectNoAccess);
        }
      });
